...
5. If no baseline exists, the current analysis is promoted as the baseline and no maintenance PRs are planned.
...
7. System creates a session report and focused PRs.
8. Every completed non-first-scan session promotes its current analysis as the latest relevant baseline.
9. User reviews report and PRs.
