...
@pytest.mark.django_db
def test_execute_tests_plan_creates_paired_test_file_without_editing_source(monkeypatch):
    from apps.maintenance_prs import ai_fixes

    source = "export function AuthLayout() {\n  return <main />\n}\n"
    generated_test = (
        "import { render } from '@testing-library/react'\n"
        "import { AuthLayout } from './AuthLayout'\n\n"
        "test('renders auth layout', () => {\n"
        "  render(<AuthLayout />)\n"
        "})\n"
    )
    prompts: list[str] = []
    ...
    plan = make_plan(
        category="tests",
        risk_tier="tier_2_assisted",
        branch_name="gardener/tests-auth-layout",
        title="Add AuthLayout coverage",
        changed_paths=["src/frontend/src/features/auth/AuthLayout.tsx"],
        required_checks=["test_review"],
    )
    client = FakeGitHubClient(
        file_contents={
            "src/frontend/src/features/auth/AuthLayout.tsx": source,
        },
        file_shas={
            "src/frontend/src/features/auth/AuthLayout.tsx": "source_sha",
        },
    )

    execute_maintenance_pr_plan(plan, client=client)

    test_path = "src/frontend/src/features/auth/AuthLayout.test.tsx"
    assert test_path in client.put_contents
    assert client.put_contents[test_path] == generated_test
    assert "src/frontend/src/features/auth/AuthLayout.tsx" not in client.put_contents
    assert "Source file: src/frontend/src/features/auth/AuthLayout.tsx" in prompts[0]
    create_pr_call = next(call for call in client.calls if call[0] == "create_pull_request")
    assert create_pr_call[-1] is True
    plan.refresh_from_db()
    assert plan.execution_status == MaintenancePRPlan.ExecutionStatus.SUCCEEDED


@pytest.mark.django_db
def test_execute_tests_plan_limits_stale_broad_plan_to_one_non_config_source(monkeypatch):
    from apps.maintenance_prs import ai_fixes

    generated_test = "test('generated by gardener', () => {})\n"
    prompts: list[str] = []
    ...
    plan = make_plan(
        category="tests",
        risk_tier="tier_2_assisted",
        branch_name="gardener/tests-broad",
        title="Plan broad tests maintenance opportunities",
        changed_paths=[
            "prisma.config.ts",
            "src/frontend/eslint.config.js",
            "src/frontend/src/features/auth/AuthLayout.tsx",
            "src/frontend/src/features/auth/AuthPanel.tsx",
        ],
        required_checks=["test_review"],
    )
    client = FakeGitHubClient(
        file_contents={
            "src/frontend/src/features/auth/AuthLayout.tsx": "export function AuthLayout() {}\n",
            "src/frontend/src/features/auth/AuthPanel.tsx": "export function AuthPanel() {}\n",
        },
        file_shas={
            "src/frontend/src/features/auth/AuthLayout.tsx": "layout_sha",
            "src/frontend/src/features/auth/AuthPanel.tsx": "panel_sha",
        },
    )
    ...
    assert "Source file: src/frontend/src/features/auth/AuthLayout.tsx" in prompts[0]
    assert set(client.put_contents) == {
        ".gardener/plans/gardener-tests-broad.md",
        "src/frontend/src/features/auth/AuthLayout.test.tsx",
    }
    assert not any("config" in str(call) for call in client.calls)
    assert "AuthPanel.test.tsx" not in client.put_contents
